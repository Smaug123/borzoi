...
/// Every plant: for each [`Form`], one per tier (a control — nothing contends,
/// so both sides must simply find it) and one per unordered pair (the contest
/// the sweep exists for).
pub fn corpus() -> Vec<Plant> {
    let mut out = Vec::new();
    for (form, prefix) in [(Form::Bare, 'T'), (Form::DottedHead, 'D')] {
...
/// The [`Form::Bare`] declarations for one tier, at `indent`.
