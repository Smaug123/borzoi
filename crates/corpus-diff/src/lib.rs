...
// project; re-exported so this crate's own callers keep naming them here.
pub use borzoi_sema::test_support::{DeclaringEntity, StructuralName};
use lsp_types::Url;
use rowan::TextRange;
...
pub struct Comparison {
    ...
    /// Which guard declined each of the two deferral counts above.
    pub decline_census: DeclineCensus,
...
pub struct CorpusSummary {
    ...
    pub assembly_deferrals: usize,
    /// Aggregate of [`Comparison::decline_census`]: which guard declined, and
    /// at which ladder tier. The deferral totals above say *how much* we lost;
    /// this says *to what*, which is what a change to the precedence ladder
    /// moves.
    pub decline_census: DeclineCensus,
    pub skipped_uses: SkippedUses,
...
impl CorpusSummary {
    ...
    pub fn record_comparison(&mut self, comparison: &Comparison) {
        ...
        self.decline_census.add_assign(&comparison.decline_census);
    ...
    pub fn render_text_report(&self) -> String {
        ...
        .expect("write String");
        let census = &self.decline_census;
        writeln!(
            out,
            "project-corpus-diff decline causes: {} attributed | {} unattributed | {}",
            census.attributed,
            census.unattributed,
            census
                .causes()
        ...
        writeln!(
            out,
            "project-corpus-diff decline tiers: {}",
            census
                .tiers()
                .iter()
                .filter(|(_, n)| **n > 0)
    ...
    fn json_report(&self) -> CorpusJsonReport<'_> {
        CorpusJsonReport {
            ...
            decline_census: CorpusDeclineCensus::of(&self.decline_census),
...
struct CorpusJsonReport<'a> {
    ...
    decline_census: CorpusDeclineCensus,
    divergences: CorpusTieredCount,
    coverage: CorpusCoverageReport,
...
pub fn render_generator_summary(
    ...
    let generator = GeneratorSummary {
        ...
        statistics: GeneratorStatistics {
            ...
            decline_census: CorpusDeclineCensus::of(&summary.decline_census),
            divergences: CorpusTieredCount {
                    ...
                    Some(expected) => {
                        ...
                        match rf.resolution_at(range) {
                            None | Some(Resolution::Deferred(_)) => {
                                ...
                                comparison.decline_census.observe(rf.decline_site(range));
                            }
...
/// [`certified_expected`](borzoi_sema::test_support::certified_expected) for an
/// oracle declaration: `None` when the oracle reported no structural name for
/// the use, so the rendered name is compared exactly as it arrived.
fn certified_expected(
    env: &AssemblyEnv,
    res: Resolution,
    expected: &AssemblyDecl,
) -> Option<String> {
    certified_structural(env, res, expected.structural.as_ref()?)
}
  ...
  "Files": [
    ...
    fn corpus_summary_aggregates_counts_and_skip_reasons() {
        ...
        let comparison = Comparison {
            ...
            decline_census: DeclineCensus::default(),
